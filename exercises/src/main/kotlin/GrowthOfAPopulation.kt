...
class GrowthOfAPopulation {
    fun nbYear(pp0:Int, percent:Double, aug:Int, p:Int): Int {
        ...
        var yearsElapsed = 0
        var currentPp0 : Int = pp0

        while (currentPp0 < p) {
            currentPp0 = (currentPp0 + (currentPp0 * percentage) + aug).toInt()
            yearsElapsed++
        }
        return yearsElapsed
    }

    fun nbYearRecursion(pp0:Int, percent:Double, aug:Int, p:Int): Int =
        if(pp0 >= p) 0 else 1 + nbYear((pp0 + pp0*percent/100 + aug).toInt(), percent, aug, p)
}